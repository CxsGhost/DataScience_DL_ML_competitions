...
class Calculation:
    ...
    def cal_loss(self, t1_, t2_, t3_, t4_, v_, gama=312):
        ...
        # 计算两个时间限制
        try:
            self.time_in_150190 = time_list[temp_list >= 190][0] - \
                                  time_list[temp_list >= 150][0]
        except :
            self.time_in_150190 = 10
        try:
            self.time_high217 = time_list[temp_list > 217][-1] - \
                                time_list[temp_list > 217][0]
        except :
            self.time_high217 = 10

        # 近似计算积分面积，面积即为最优化目标，返回该值用于进化算法求解
        s_list = time_list[np.argmax((temp_list >= 217).astype(np.int)):
                           np.argmax(temp_list) + 1]
        s = 0
        for p in range(len(s_list) - 1):
            s += 0.5 * (s_list[p] + s_list[p + 1]) / 2
        return s

    # 处理限制条件，把化为7个不等式条件，对约束条件的违反度度量可行度
    def cal_cv(self):
...
# 构造使用进化算法求解的单目标带约束的多变量优化问题
class MyProblem(ea.Problem):
    def __init__(self):
        name = 'MyProblem'
        M = 1  # 初始化目标维数
        max_or_min = [1]  # 目标最小化标记
        Dim = 5  # 初始化决策变量维数
        varTypes = [0] * Dim  # 初始化决策变量的类型，元素为0表示对应的变量是连续的

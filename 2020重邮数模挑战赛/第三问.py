...
def sum_time(seq_path_, zero_pos_):
    ...
    if len(path_):
        if len(path_) == 1:
            time_ = (distance_mat[0][path_[0]] * 2) / v * 60 + 8
...
while tk > te:
    ...
    cur_ze = np.sum(time_info[1])
    excess_list = weight_info[2]
    for item in excess_list:
        cur_ze += distance_mat[0][item[0]] / 25 * 60 + distance_mat[0][item[1]] / 25 * 60
        cur_ze -= distance_mat[item[0]][item[1]] / 25 * 60
    ...
    if cur_ze <= best_ze:
        ...
        if np.random.rand(1)[0] < np.exp(-beta_k(tk, te, t0)):
            ...
            better_ze.append(best_ze / 60 * 25)
...
def divide_path(best_path_):
    ...
    path = seq_path_[: zero_pos_[0]]
    if len(path):
        divide_ways.append(copy.deepcopy(path))
    for w in range(len(zero_pos_) - 1):
        path = seq_path_[zero_pos_[w]: zero_pos_[w + 1]]
        if len(path):
            divide_ways.append(copy.deepcopy(path))
    path = seq_path_[zero_pos_[-1]:]
    if len(path):
        divide_ways.append(copy.deepcopy(path))
    ...
    print(sum_time(seq_path, zero_pos)[1])
    return final_way

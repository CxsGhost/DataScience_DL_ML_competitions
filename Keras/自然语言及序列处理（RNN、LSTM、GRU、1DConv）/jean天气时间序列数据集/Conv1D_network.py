...
network.add(layers.Conv1D(32, 5, activation=activations.relu, input_shape=(240, float_data.shape[-1])))
network.add(layers.MaxPool1D(3))
network.add(layers.Conv1D(32, 5, activation=activations.relu))
network.add(layers.MaxPool1D(3))
network.add(layers.Conv1D(32, 5, activation=activations.relu))

# 在steps维度（也就是第二维）对整个数据求最大值
# 比如说输入数据维度是[10, 4, 10]，那么进过全局池化后，输出数据的维度则变成[10, 10]
network.add(layers.GlobalMaxPool1D())

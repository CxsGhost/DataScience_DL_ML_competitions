...
"""
直接把labels按原标签编码为张量整数, 但是要改一下损失函数成sparse_c_c。
一般来说分类编码(one-hot编码）用c_c，直接整数编码就用s_c_c
"""
...
model.add(layers.Dense(46, activation='softmax'))  # 即便是label按原本输入，model可以自动从46个选出预测的类

model.compile(optimizer=optimizers.rmsprop(lr=0.001),
              loss=losses.sparse_categorical_crossentropy,  # 这个与categorical_crossentropy数学上一样，只接口不同
